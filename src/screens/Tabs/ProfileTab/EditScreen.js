...
function EditScreen({ navigation }) {
  const [date, setDate] = useState(
    navigation.getParam("birthday")
      ? new Date(navigation.getParam("birthday"))
      : new Date()
  );
  ...
  const HandleSave = () => {
    ...
    if (!ValidNumber(weight) || weight === "") {
      Alert.alert("Oops!", "Weight only enter numbers");
      return;
    }
    if (date.getFullYear() === new Date().getFullYear()) {
      Alert.alert("Oops!", "Invalid date of birth");
      return;
    }
      ...
      <KeyboardAvoidingView behavior="position" enabled={enableshift}>
        <ScrollView>
          ...
          <TextFieldInput
            ...
            text={job}
          />
          <DateFieldInput title="Birthday" date={date} setDate={setDate} />
          <TextFieldInput
