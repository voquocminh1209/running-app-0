...
export default class SaveActivityScreen extends React.Component {
  ...
  handleSaveActivity = () => {
    ...
    // send post request
    fetch(options).catch((err) => console.log("dòng 94 saveAct: " + err));
  ...
  fetchDataUser = () => {
    AsyncStorage.getItem("authToken").then(async (token) => {
      ...
      Axios.get(
        ...
        .then((res) => {
          if (
            this.checkNullUndefined(res.data.weight) &&
            res.data.weight != 0
          ) {
  ...
  onPress_btnEnterWeight = () => {
    ...
    if (
      !this.validNumber(weight) ||
      !this.checkNullUndefined(weight) ||
      weight == 0
    ) {
      Alert.alert("Oops!", "Height only enter numbers");
    } else {
      this.calcBurnedCalories();
      this.rounding();
  ...
  defaultTitle = () => {
    ...
    if (!this.checkNullUndefined(title)) {
      if (
        Date.parse(currentTime) > Date.parse("01/01/2011 00:00:00") &&
    ...
    return isDataUserLoading ? (
      isValidWeight ? (
        ...
        >
          ...
          >
            <Text style={[styles.buttonTitle, { color: "white" }]}>Enter</Text>
          </TouchableOpacity>
        </View>
      )
    ) : (
      ...
      >
        ...
        <Text style={styles.title}>Your achievement</Text>
        ...
        <View style={styles.containerRecord}>
          ...
          <View style={styles.itemRecord}>
            ...
            <Text style={styles.itemTitle}>AVG PACE</Text>
          </View>
        </View>

        <Text style={styles.title}>Your running route</Text>
        ...
        <Text style={styles.title}>How did that activity feel?</Text>
        ...
        <View style={styles.buttonRow}>
          ...
          >
            <Text style={[styles.buttonTitle, { color: "white" }]}>SAVE</Text>
          </TouchableOpacity>
        </View>
      </ScrollView>
    );
