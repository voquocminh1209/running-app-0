...
function Register({ navigation }) {
  ...
  const handleRegister = () => {
    ...
    if (!ValidUser(username)) {
      Alert.alert("Oops!", "Username không hợp lệ!");
      return 0;
    }
    if (!ValidateEmail(mail)) {
      Alert.alert("Oops!", "Email không hợp lệ");
      return 0;
    }
    ...
    if (password != RePassword) {
      Alert.alert("Oops!", "Bạn đã nhập sai RePassword");
      return 0;
    }
    ...
    Axios.post(
      ...
      .catch((err) => {
        console.log(err);
        Alert.alert("Oops! :(", "Username đã tồn tại!!");
      });
  };
